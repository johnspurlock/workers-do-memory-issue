...
export async function putScript(accountId: string, scriptName: string, scriptContents: Uint8Array, bindings: Binding[], apiToken: string): Promise<Script> {
    ...
    const metadata = { 'main_module': 'main', bindings };
...
async function execute(op: string, method: 'GET' | 'POST' | 'PUT', url: string, apiToken: string, body?: string /*json*/ | FormData): Promise<CloudflareApiResponse> {
